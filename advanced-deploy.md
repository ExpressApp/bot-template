...
## Продвинутая инструкция по развертыванию bot_refactor
...
```sql
CREATE USER bot_refactor_user PASSWORD '<GENERATE>';
CREATE DATABASE bot_refactor_db;
GRANT ALL PRIVILEGES ON DATABASE bot_refactor_db 
  TO bot_refactor_user;
```

2. Скачайте репозиторий на сервер:

```bash
git clone <THIS_REPOSITORY> /opt/express/bots/bot_refactor
cd /opt/express/bots/bot_refactor
```

3. Соберите образ:

```bash
docker build -t bot_refactor .
```
...
4. В директории `/opt/express/bots/bot_refactor` создайте файл
